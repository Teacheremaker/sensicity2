...
-- Table de la main courante (updated with missing columns)
CREATE TABLE IF NOT EXISTS logbook_entries (
    ...
    event_type VARCHAR(255) NOT NULL DEFAULT 'Observation',
    priority VARCHAR(50) CHECK (priority IN ('low', 'medium', 'high', 'critical')) NOT NULL DEFAULT 'medium',
    location TEXT,
    status VARCHAR(50) CHECK (status IN ('new', 'in_progress', 'closed', 'awaiting_requisition')) NOT NULL DEFAULT 'new',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Table des actions de la main courante
